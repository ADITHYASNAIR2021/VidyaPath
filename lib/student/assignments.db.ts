...
export interface StudentAssignmentsSession {
  classLevel: 10 | 12;
  schoolId: string;
  section?: string;
...
function buildAllowedSubjectSet(session: StudentAssignmentsSession): Set<string> {
...
export async function listVisibleAssignmentsForStudent(
  session: StudentAssignmentsSession
): Promise<TeacherAssignmentPack[]> {
  const studentSection = toStudentSection(session.section);
  const allowedSubjects = buildAllowedSubjectSet(session);

  const teacherRows = await supabaseSelect<TeacherProfileRow>('teacher_profiles', {
    select: 'id,school_id,status',
    filters: [
      { column: 'school_id', value: session.schoolId },
  ...
  const assignmentRows =
    schoolTeacherIds.size === 0
      ...
      : await supabaseSelect<TeacherAssignmentPackRow>('teacher_assignment_packs', {
          ...
          filters: [
            { column: 'class_level', value: session.classLevel },
  ...
  return assignmentRows
    ...
    .filter((row) => {
      ...
      return rowSchoolId === session.schoolId;
    })
