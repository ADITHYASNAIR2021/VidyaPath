    ...
    <div className="min-h-screen bg-[#FDFAF6] dark:bg-navy-900 text-[#1C1C2E] dark:text-gray-100">
      ...
      <div className="mx-auto max-w-7xl px-3 sm:px-4 py-6">
        <div className="sticky top-16 z-20 bg-[#FDFAF6]/95 dark:bg-navy-900/95 backdrop-blur pb-3">
          <div className="mb-5 rounded-2xl border border-[#E8E4DC] dark:border-gray-700 bg-white dark:bg-gray-900 p-3 sm:p-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-xl border border-[#E8E4DC] dark:border-gray-700 bg-white dark:bg-gray-950 py-2.5 pl-9 pr-3 text-sm text-[#1C1C2E] dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-400"
            />
          </div>
                  ...
                  <button
                    ...
                    className={`snap-start whitespace-nowrap rounded-full border px-3 py-1.5 text-xs ${
                      selectedSubject === subject
                        ...
                        : 'border-indigo-200 dark:border-indigo-700 bg-white dark:bg-gray-950 text-indigo-700 dark:text-indigo-300'
                    }`}
                  >
                    {subject}
                  </button>
                  ...
                  onClick={() => setSelectedClass(classLevel)}
                    className={`rounded-full border px-3 py-1.5 text-xs ${
                      selectedClass === classLevel
                        ...
                        : 'border-emerald-200 dark:border-emerald-700 bg-white dark:bg-gray-950 text-emerald-700 dark:text-emerald-300'
                    }`}
                  >
              ...
              <button
                ...
                className={`rounded-full border px-3 py-1.5 text-xs ${
                  jeeOnly
                    ...
                    : 'border-amber-200 dark:border-amber-700 bg-white dark:bg-gray-950 text-amber-700 dark:text-amber-300'
                }`}
              >
                JEE equations only
              </button>
        ...
        <div className="grid gap-5 xl:grid-cols-[1fr_280px]">
          <div className="min-w-0 space-y-4">
            {SUBJECTS.filter((subject) => subject !== 'All' && grouped.has(subject)).map((subject) => {
              ...
              return (
                <section key={subject} className="rounded-2xl border border-[#E8E4DC] dark:border-gray-700 bg-white dark:bg-gray-900 p-4">
                  <h2 className="font-fraunces text-xl font-bold text-navy-700 dark:text-gray-100">{subject}</h2>
                  <div className="mt-3 space-y-3">
                    {[...chapters.entries()]
                      ...
                      .map(([chapterKey, equations]) => {
                        ...
                        return (
                          <div key={chapterKey} className="min-w-0 rounded-xl border border-[#E8E4DC] dark:border-gray-700 bg-[#FAF9F5] dark:bg-gray-950 p-3">
                            <div className="flex items-center justify-between gap-2">
                              <p className="text-sm font-semibold text-navy-700 dark:text-gray-100">{chapterTitle}</p>
                              <Link href={`/chapters/${chapterId}`} className="text-xs font-semibold text-indigo-700 dark:text-indigo-300 hover:text-indigo-800 dark:hover:text-indigo-200">
                                Open chapter
                              </Link>
                            </div>
                            <div className="mt-2 grid min-w-0 gap-2 md:grid-cols-2">
                              {equations.map((entry) => (
                                <div key={entry.id} className="min-w-0 rounded-lg border border-[#E8E4DC] dark:border-gray-700 bg-white dark:bg-gray-900 px-2.5 py-2">
                                  ...
                                  <p className="mt-1 text-[11px] text-[#6E6984] dark:text-gray-400">Unit hint: {entry.siUnitHint}</p>
          ...
          <aside className="h-fit rounded-2xl border border-[#E8E4DC] dark:border-gray-700 bg-white dark:bg-gray-900 p-4">
            <h3 className="font-fraunces text-lg font-bold text-navy-700 dark:text-gray-100">Formula Sources</h3>
            <p className="mt-1 text-xs text-[#6E6984] dark:text-gray-400">Handbook and official source references.</p>
            <div className="mt-3 space-y-2">
              {FORMULA_SOURCE_DOCS.map((doc) => (
                <div key={`${doc.subject}-${doc.sourceName}`} className="rounded-lg border border-[#E8E4DC] dark:border-gray-700 bg-[#FAF9F5] dark:bg-gray-950 px-2.5 py-2">
                  <p className="text-xs font-semibold text-[#1F1F35] dark:text-gray-100">
                    {doc.sourceUrl ? (
                      <a href={doc.sourceUrl} target="_blank" rel="noopener noreferrer" className="text-indigo-700 dark:text-indigo-300 hover:text-indigo-800 dark:hover:text-indigo-200">
                        {doc.sourceName}
                      </a>
                    ) : (
                      doc.sourceName
