...
export default function CalendarPage() {
  ...
  const [form, setForm] = useState({
    title: '',
    type: 'assignment_due' as EventType,
    eventDate: todayIso(),
    description: '',
    classLevel: '' as '' | '10' | '12',
    section: '',
  ...
  async function loadEvents() {
    ...
    try {
      ...
      if (!eventsRes.ok) {
        setError(body?.message || 'Failed to load calendar events.');
        setEvents([]);
        return;
      }
  ...
  async function deleteEvent(id: string) {
    try {
      const response = await fetch(`/api/teacher/calendar?id=${encodeURIComponent(id)}`, { method: 'DELETE' });
      if (!response.ok) {
        const body = await response.json().catch(() => null);
    ...
    <div className="mx-auto max-w-5xl p-6">
      ...
      <div className="mb-6 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500">Manage class events, deadlines, meetings, and exam reminders.</p>
        </div>
        <button
            ...
            <div key={event.id} className="rounded-2xl border border-[#E8E4DC] bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  <span className={clsx('rounded-full border px-2 py-1 text-[11px] font-semibold capitalize', EVENT_COLORS[event.type])}>
