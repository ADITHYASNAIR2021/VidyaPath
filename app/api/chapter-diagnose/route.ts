  ...
  try {
    ...
    if (!parsed) {
      return NextResponse.json(
        { error: 'Invalid request. Required: { chapterId, quizScore?, flashcardsDue?, studied?, bookmarked?, recentMistakes? }' },
        { status: 400 }
      );
    }
    ...
    if (!fallback) {
      return NextResponse.json({ error: 'Unable to diagnose chapter.' }, { status: 500 });
    }
...
- Output JSON only.`,
    ...
    return NextResponse.json({ error: 'Failed to diagnose chapter.' }, { status: 500 });
  }
}
