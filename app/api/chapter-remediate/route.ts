...
export async function POST(req: Request) {
  ...
  try {
    ...
    const limit = await checkRateLimit({
      key: buildRateLimitKey('ai:chapter-remediate', [context?.authUserId || getClientIp(req), context?.schoolId]),
      windowSeconds: 60,
      maxRequests: 10,
      blockSeconds: 120,
    });
    ...
    const bodyResult = await parseAndValidateJsonBody(req, 48 * 1024, chapterRemediateRequestSchema);
    if (!bodyResult.ok) {
      return errorJson({
        requestId,
    ...
    const tokenBudget = await checkAiTokenBudget({
      ...
      projectedOutputTokens: 1600,
    });
    if (!tokenBudget.allowed) {
      return errorJson({
        requestId,
        errorCode: tokenBudget.reason || 'token-cap-exceeded',
        message: 'AI usage limit reached for chapter remediation planning.',
        status: 429,
